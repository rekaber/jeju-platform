"""
제주 연립/다세대 매매 실거래 데이터 수집기 v1
- 기간: 2024년 1월 ~ 2026년 8월 (32개월)
- 지역: 제주시(50110) + 서귀포시(50130)
- 저장: jeju_rht_trades.db (SQLite)
- API: 국토교통부 연립다세대 매매 실거래가 자료 (신규 API, 영문 태그)
  endpoint: getRTMSDataSvcRHTrade
  태그: umdNm, mhouseNm, excluUseAr, landAr, dealYear/Month/Day, dealAmount 등

실행 방법:
  pip install requests
  python fetch_rht_data.py
"""
...
START_YM  = "202401"
END_YM    = "202608"
DB_PATH   = os.path.join(os.path.dirname(os.path.abspath(__file__)), "jeju_rht_trades.db")
DELAY_SEC = 0.6
MAX_RETRY = 3
RETRY_DELAY = 5.0
# ────────────────────────────────────────────────────────────
...
def init_db(conn):
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_trade_date ON rht_trades(trade_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_dong       ON rht_trades(dong)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_price      ON rht_trades(price_man)")
    conn.commit()
...
def fetch_month(lawd_cd: str, ym: str, attempt: int = 1):
    ...
    for item in root.findall(".//item"):
        ...
        rows.append({
            ...
            "house_type": g("houseType"),   # 연립 or 다세대
...
def save_rows(conn, rows, sigungu, lawd_cd, fetched_at):
    conn.executemany("""
        INSERT INTO rht_trades
            (sigungu, lawd_cd, dong, house_name, jibun, area, land_ar,
             price_man, price_eok, floor, build_year, house_type,
             deal_type, cancel_yn, cancel_day, seller_type, buyer_type,
             reg_date, year, month, day, trade_date, fetched_at)
        VALUES
            (:sigungu, :lawd_cd, :dong, :house_name, :jibun, :area, :land_ar,
             :price_man, :price_eok, :floor, :build_year, :house_type,
             :deal_type, :cancel_yn, :cancel_day, :seller_type, :buyer_type,
...
def main():
    ...
    print(f"{'═'*50}")
    print(f"  제주 연립/다세대 실거래 데이터 수집기 v1")
